...
public class NodeAsyncPopulate extends AsyncTask<Void, Void, Void> {
    ...
    @Override
    protected Void doInBackground(final Void... params) { //called during database initialization, loads in all data
        nDao.DeleteAll(); //clear anything that's somehow already there (Room can crash if we try inserting the same thing twice)
        MapNode startNodes[] = new MapNode[]{new MapNode(450,200, 3, "Walker", "hallway"), new MapNode(450,550, 3, "Walker", "hallway"),
