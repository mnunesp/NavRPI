...
public abstract class BuildingDatabase extends RoomDatabase {
    ...
    static BuildingDatabase getDatabase( final Context context) { //method to construct the database, a singleton class
        if (INSTANCE == null) {
            synchronized (BuildingDatabase.class) { //this stuff checks to make sure we only ever make one
                if (INSTANCE == null) {
